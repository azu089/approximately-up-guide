...
const runBuild = fixture => {
  ...
  delete env.APPROX_UP_ADSENSE_FIXTURE;
  if (fixture) {
    env.NODE_ENV = "test";
    env.APPROX_UP_ADSENSE_FIXTURE = "enabled";
  }
...
assert.equal(site.site.languages.length, 14, "fourteen-language configuration changed");
assert.equal(site.site.adsenseId, expectedPublisher, "generated AdSense publisher data must stay raw pub-");
...
assert.equal(buildSource.includes(`"adsenseId": "${expectedPublisher}"`), true,
  "canonical build_content.py lost the raw AdSense publisher ID");
for (const gate of ["enabled", "providerReady", "certifiedCmpReady"]) {
  assert.equal(new RegExp(`"${gate}"\\s*:\\s*False`).test(buildSource), true,
    `canonical ${gate} gate must default false`);
}
...
assert.equal(indexableFiles.length, site.site.languages.length * (site.pages.length + 4), "fourteen-language route set is incomplete");
